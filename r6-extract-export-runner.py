...
def process_json_to_xlsx(json_path):
    ...
    ws_stats.append(['TEAMS', 'PLAYERS', 'Headshot %'])  # Column headers

    # Process the first match to get team names, players, and their headshot percentages
    first_match = data['rounds'][-1] if data['rounds'] else None  # Use the last round for final stats
    if first_match:
        ...
        headshot_percentages = [f"{player.get('headshotPercentage', 0)}%" for player in players]

        # Write team names, player names, and headshot percentages to the Stats sheet
        ...
        for i in range(max_length):
            ...
            headshot_percentage = headshot_percentages[i] if i < len(headshot_percentages) else "0%"
            ws_stats.append([team_name, player_name, headshot_percentage])
